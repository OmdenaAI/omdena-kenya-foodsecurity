...
#try to link/join my 'id_menage'..... (see below)
...
#----------------------------------------------------------------#
#back to ID_menage merging/sorting/sql attempts
length(unique(households$ID_menage))
length(is.na(households$ID_menage))
which(is.na(households$ID_menage))
colnames(central_zone)
length(central_zone$ID_menage)
length(unique(central_zone$ID_menage))
#i want to merge these 576 id menage from the producers, production, and captial data set
df = merge(central_zone, producers, by="ID_menage")
head(df)
length(df$ID_menage)
length(unique(df$ID_menage))
#that has reduced my producers set from 2951 to 978 - 576 which are unique
#let me do the same for the production and capital
df1 = merge(central_zone, production, by="ID_menage") #from 3304 to 1012
df2 = merge(central_zone, capital, by="ID_menage") #from 5869 to 2041
#let me just select original and lat/lon columns from these df, df1, d2
df = select(df, 1, 'gpslat','gpslon',119:129) #central + producers
df1 = select(df1, 1, 'gpslat','gpslon',119:498) #central + production
df2 = select(df2, 1, 'gpslat','gpslon',119:157) #cenral + capital
length(unique(df$ID_menage)) #576 ok
length(unique(df1$ID_menage)) #569 , some missing data
length(unique(df2$ID_menage)) #576 ok

hist(df$G04, breaks=4)
ggplot(df, aes(x=G04)) + 
  geom_histogram(aes(y=..density..), colour="black", fill="gray", binwidth = 10)+
  geom_density(alpha=.2, fill="#69b3a2") + 
  geom_vline(aes(xintercept=mean(G04)),color="blue", linetype="dashed", size=1)
gender <- as.character(df$G03) 

df3 <- cbind(df,gender)

ggplot(df3, aes(x = G04)) +
  geom_histogram(aes(color = gender, fill = gender), 
                 position = "identity", bins = 10, alpha = 0.4) +
  scale_color_manual(values = c("darkolivegreen4", "firebrick3")) +
  scale_fill_manual(values = c("darkolivegreen4", "firebrick3"))
...
length(southern_zone$ID_menage)
length(unique(southern_zone$ID_menage))
df = merge(southern_zone, producers, by="ID_menage")
df1 = merge(southern_zone, production, by="ID_menage") #from 3304 to 1012
df2 = merge(southern_zone, capital, by="ID_menage") #from 5869 to 2041
#let me just select original and lat/lon columns from these df, df1, d2
df = select(df, 1, 'gpslat','gpslon',119:129) #central + producers
df1 = select(df1, 1, 'gpslat','gpslon',119:498) #central + production
df2 = select(df2, 1, 'gpslat','gpslon',119:157) #cenral + capital
length(unique(df$ID_menage)) #815 ok
length(unique(df1$ID_menage)) #781 , some missing data
length(unique(df2$ID_menage)) #815 ok

hist(df$G04, breaks=10)
ggplot(df, aes(x=G04)) + 
  geom_histogram(aes(y=..density..), colour="black", fill="gray", binwidth = 10)+
  geom_density(alpha=.2, fill="#69b3a2") + 
  geom_vline(aes(xintercept=mean(G04)),color="blue", linetype="dashed", size=1)
...
two_region <- central_map + southern; two_region

#... need to come back and aalyze these datsets more... (12/27/20)
